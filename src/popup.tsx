...
const Popup = () => {
  ...
  const [isCurrentSiteDistracting, setIsCurrentSiteDistracting] = useState(false);
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      if (tabs[0]?.url) {
        ...
        chrome.storage.sync.get(['distractingSites'], (result) => {
          ...
          const isDist = savedSites.some((site: DistractingSite) => 
            domain.includes(site.domain) || site.domain.includes(domain.replace('www.', ''))
          );
  ...
  const toggleDistractingSite = () => {
    ...
    if (isCurrentSiteDistracting) {
      ...
      const updatedSites = distractingSites.filter(site => 
      ...
      <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border-b border-slate-200 dark:border-slate-700 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div>
              <h1 className="text-lg font-bold text-slate-800 dark:text-white">Waitful</h1>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-xl p-4 border border-slate-200/50 dark:border-slate-700/50 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Today's Usage</h3>
          </div>
          
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <span className="text-sm text-slate-600 dark:text-slate-400">Distracting sites</span>
              <span className="text-sm font-bold text-slate-800 dark:text-slate-200">{timeSpentToday}</span>
            </div>
            ...
            <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400">
              <span>0m</span>
              <span className={timePercentage > 80 ? 'text-red-500 font-medium' : ''}>
        ...
        <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-xl p-4 border border-slate-200/50 dark:border-slate-700/50 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            <img 
              ...
              onError={(e) => {
                (e.target as HTMLImageElement).src = 'data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjQiIGhlaWdodD0iMjQiIHZpZXdCb3g9IjAgMCAyNCAyNCIgZmlsbD0ibm9uZSIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj4KPHJlY3Qgd2lkdGg9IjI0IiBoZWlnaHQ9IjI0IiByeD0iNCIgZmlsbD0iIzk0YTNiOCIvPgo8L3N2Zz4K';
              }}
            />
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {isCurrentSiteDistracting ? 'Marked as distracting' : 'Current website'}
              </p>
            </div>
          </div>
        ...
        {distractingSites.length > 0 && (
          <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-xl p-4 border border-slate-200/50 dark:border-slate-700/50 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate-700 dark:text-slate-300">Blocked Sites</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">Currently tracking {distractingSites.length} sites</p>
              </div>
              <div className="text-right">
                <p className="text-lg font-bold text-blue-600 dark:text-blue-400">{distractingSites.length}</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">sites</p>
              </div>
            </div>
          </div>
        )}
